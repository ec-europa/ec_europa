<?php

/**
 * @file
 * Display Suite 3 column 25/50/25 stacked template.
 */
?>
<<?php print $layout_wrapper; print $layout_attributes; ?> class="ecl-container <?php print $classes;?>">

  <?php if (isset($title_suffix['contextual_links'])): ?>
  <?php print render($title_suffix['contextual_links']); ?>
  <?php endif; ?>

  <<?php print $header_wrapper ?> class="ecl-row<?php print $header_classes; ?>">
    <?php print $header; ?>
  </<?php print $header_wrapper ?>>

  <<?php print $ds_content_wrapper; print $layout_attributes; ?> class="ecl-row <?php print $classes;?>">
    <<?php print $left_wrapper ?> class="ecl-col-3<?php print $left_classes; ?>">
      <?php print $left; ?>
    </<?php print $left_wrapper ?>>

    <<?php print $middle_wrapper ?> class="ecl-col-6<?php print $middle_classes; ?>">
      <?php print $middle; ?>
    </<?php print $middle_wrapper ?>>

    <<?php print $right_wrapper ?> class="ecl-col-3<?php print $right_classes; ?>">
      <?php print $right; ?>
    </<?php print $right_wrapper ?>>

  </<?php print $ds_content_wrapper ?>>

  <<?php print $footer_wrapper ?> class="ecl-row<?php print $footer_classes; ?>">
    <?php print $footer; ?>
  </<?php print $footer_wrapper ?>>

</<?php print $layout_wrapper ?>>

<?php if (!empty($drupal_render_children)): ?>
  <?php print $drupal_render_children ?>
<?php endif; ?>
